                    ...
                    <nav id="docs-nav" class="docs-nav navbar" >
                        @foreach ($modules as $mod )
                            <ul class="section-items list-unstyled nav flex-column pb-3">
                                <li class="nav-item section-title"><a style="a:hover{color:#f3b052}" href="{{ route('view.show',$mod->id) }}"><span class="theme-icon-holder me-2">{!!$mod->flatte_icon!!}</span>{{$mod->name}}</a></li>

                                @foreach ($posts as $post)
                                    @if ($mod->id == $post->help_module_id)
                                        <li class="nav-item"><a class="nav-link scrollto" href="{{ route('view.oneshow',$post->id) }}">{{$post->titre}}</a></li>
                                    @endif
                                @endforeach
                           </ul>
                        @endforeach
